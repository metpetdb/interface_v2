...
def handle_fields(filters,sample_search):
    ...
    if type(filters['fields']) is str: # chem search
        print 'before list -->',filters['fields']
        filters['fields'] = [filters['fields']]
        print 'after list -->',filters['fields']

    # if sorting_name not in filters['fields'][0]: # always show field used for sort
    #     filters['fields'][0] += ','
    #     filters['fields'][0] += sorting_name
    # flip args back for pages

    if filters['fields'][0].split(',')[0] == 'id': # second page
        # get title values back from variables
